...
type ViewState =
  ...
  | { kind: 'unsupported'; empresa: EmpresaSanitaria }

export function ResultViewAgua({ empresaSlug }: { empresaSlug: string }) {
  ...
  useEffect(() => {
    ...
    try {
      const boleta = parseAgua(payload.empresa, payload.rawText)
  ...
  if (state.kind === 'parser-error') {
    return (
      <ErrorView
        ...
        description="El parser detectó la sanitaria pero no logró extraer los datos. Prueba con la versión digital del PDF (no escaneada)."
        detail={state.error}
      />
    )
  }
  ...
  if (state.kind === 'unsupported') {
    return (
      <ErrorView
        ...
        description={`El parser de ${state.empresa} todavía no está implementado, necesitamos una boleta real para mapear el formato exacto. Mientras tanto puedes volver a la pantalla de subida o probar con otra herramienta.`}
      />
    )
  }
  ...
  function handleAddText(newText: string) {
    ...
    try {
      const updatedBoleta = parseAgua(state.payload.empresa, combinedText)
  ...
  return (
    <main className="flex-1">
      <section className="bg-cream py-12 md:py-16">
        <Container>
          ...
          {hasFlags ? (
            ...
            <p className="mt-6 max-w-2xl text-lg leading-relaxed text-body">
              Revisamos cada línea contra las tarifas SISS vigentes de tu
              sanitaria. No encontramos cargos sospechosos esta vez.
            </p>
          )}
        </Container>
      </section>
        ...
        <Container>
          <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap">
            ...
            <Button asChild variant="ghost" size="lg">
              <Link href="/boleta-agua">Subir otra boleta</Link>
            </Button>
          </div>
        </Container>
      </section>
      ...
      <ComparativaSection
        empresa={boleta.empresa as EmpresaSanitaria}
        actual={boleta}
        slug={payload.slug}
      ...
      <section className="bg-cream py-16">
        <Container>
          ...
          <p className="mt-10 max-w-2xl text-xs text-soft">
            Herramienta referencial. Para reclamos formales: SERNAC, SISS. El
        ...
        <Container>
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <Button asChild variant="dark" size="lg">
              <Link href="/boleta-agua">Volver a subir</Link>
            </Button>
          </div>
        </Container>
      </section>
