...
export function UploadHubGas() {
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      if (parser.servicio !== 'gas') {
        throw new Error(
          `Detectamos que esto parece una boleta de ${parser.servicio} (${parser.empresa}), no de gas. Subila en la página correspondiente.`,
        )
      }

      const empresa = parser.empresa as EmpresaGas
      const slug = GAS_SLUGS[empresa]
      if (!slug) {
        throw new Error(
      ...
      {error && (
        <div className="mt-6">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <Button asChild variant="ghost" size="sm">
              <Link href="/boleta-agua">Ver boleta de agua</Link>
            </Button>
          </div>
        </div>
      )}
